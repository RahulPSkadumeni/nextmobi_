...
router.get("/place-order", verifyLogin, async (req, res, next) => {
  ...
  if (cartCount != 0) {
    ...
    if (totalPrice > 5000) {
      ...
      couponPrice = await couponHelpers.getCouponPrice(
    ...
    //   });
    walletBalance = await userHelpers.walletAmountCheckForUser(
      req.session.user._id
    );
...
router.post("/place-order", async (req, res) => {
  ...
  userHelpers
    .placeOrder(req.body, product, totalPrice, userId)
    .then(async (orderId) => {
      console.log(
      ...
      if (req.body["payment-method"] == "COD") {
        ...
        console.log("======= wallet ======");
        let walletAmount = await userHelpers.walletAmountCheck(
          req.session.user._id,
          totalPrice
        );
        if (walletAmount) {
          console.log(walletAmount);
          paymentWallet = await userHelpers.WalletAmountReduce(
            req.session.user._id,
            totalPrice,
            walletAmount
          );
...
router.get("/pay/:id", (req, res) => {
  const create_payment_json = {
    ...
    redirect_urls: {
      return_url: "http://nextmobi.tech/paypalorder-success/" + req.params.id,
      cancel_url: "http://nextmobi.tech/cancel",
    },
    transactions: [
      {
        amount: {
...
router.post("/add-address/", async (req, res) => {
  console.log(req.body);
  let addAdress = await userHelpers.addNewAdress(
    req.body,
    req.session.user._id
  );
...
router.post("/edit-address/:id", async (req, res) => {
  ...
  console.log(req.params.id);
  // let address= await userHelpers.getAddressEdit(req.params.id)
  let updateAdress = await userHelpers.updateAddress(
    req.body,
    req.params.id,
    req.session.user._id
  );
...
router.post("/addToWishList", (req, res) => {
  if (req.session.loggedIn) {
    ...
    userHelpers
      .addToWishList(req.body.id, req.session.user._id)
      .then((response) => {
        res.json({ login: true });
      });
  } else {
...
router.post("/enter-coupon", async (req, res) => {
  try {
    console.log(
      "LLLLLLLLLLLLLLLLLLLLLLLcouponLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL"
    );
    console.log(req.body);
    let code = req.body.code;
    let total = await userHelpers.getTotalAmout(req.session.user._id);
    console.log("total" + total);
    let applyCoupon = await couponHelpers.applyCoupon(
      req.body,
      total,
      req.session.user._id
    );
...
router.get("/productlist", async function(req, res, next) {
  ...
  console.log(
    "hajajajjajakakkakakakakkalalalalalllllllalallllllllllllllllllllllllllllll"
  );
  ...
  results.pageCount = Math.ceil(
    parseInt(productsCount) / parseInt(limit)
  ).toString();
