...
module.exports = {
  ...
  doLogin: (userData) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let acessStatus = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ acessStatus: true });
        ...
        .aggregate([
...

          {
            $project: {
              ...
              discountOff: {
                $cond: {
                  if: { $gt: ["$product.discount", "$category.offer"] },
                  then: { $toInt: "$product.discount" },
                  else: { $toInt: "$category.offer" },
                },
              },
            },
          },
    ...
    return new Promise((resolve, reject) => {
      if (count == -1 && quantity == 1) {
        ...
        db.get()
          .collection(collection.CART_COLLECTION)
          .updateOne(
            { _id: objectId(cartId), "product.item": objectId(productId) },
            {
  ...
  placeOrder: (order, product, total, userId) => {
    return new Promise(async (resolve, reject) => {
      ...
      db.get()
        .collection(collection.ORDER_COLLECTIOIN)
        .insertOne(orderObj)
        .then((response) => {
          console.log(
            ">>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<GGGGGGGGGGGG"
  ...
  deleteAddress: (addessId) => {
    // console.log("haiii");
    return new Promise(async (resolve, reject) => {
      let deleteAddress = await db
        .get()
        .collection(collection.ADDRESS_COLLECTION)
        .deleteOne({ _id: objectId(addessId) });
      //    console.log(response)
  ...
  addToWishList: (proId, userId) => {
    console.log(
      "nvnvnvnvnvnvnnnnnnnnnnnn..................,,,,,,,,,,,,,,,,,,,,,,"
    );
    let product = {
      item: objectId(proId),
    };
    return new Promise(async (resolve, reject) => {
      let findWishList = await db
        .get()
        .collection(collection.WISHLIST_COLLECTION)
        .findOne({ user: objectId(userId) });
      console.log(findWishList);
      if (findWishList) {
        let prodExist = findWishList.products.findIndex(
          (product) => product.item.toString() == objectId(proId).toString()
        );
